  ...
  '@aether/dex-editor': [
    ...
    McpToolDefinition(
      ...
      description:
          ...
          '- target=overview：一次聚合搜索。DEX 面（class/method/field/string）总是执行；'
          ...
          '- target=strings：DEX 字符串池（用 filter 过滤）。若字符串被 const-string 引用，'
          '会按引用类各出一条（className 点分 + locator=dex_class:...），referencedBy 列全部引用类，'
          '便于定位哪些类引用了该字符串；仅作类型/方法/字段名出现的字符串无归属；\n'
          '- target=files：APK 内文本文件搜索（用 query 作为 pattern，命中含 locator=apk_file:路径:行号）；\n'
          '- target=arsc：resources.arsc 搜索（arscTarget=strings 命中含 locator=arsc_string:下标；'
          'arscTarget=resources 命中含'
