...
/// memories are provided to the model: the injection mode plus the advanced
/// numeric knobs (token 预算 / topK / 全量阈值). Everything reads and writes
/// [MemorySettingsController] (a single JSON blob in the KV store), so changes
/// take effect on the next turn.
///
/// 嵌入模型选择与向量检索属于后续阶段（当前为全量注入），故本页只暴露注入方式与高级参数。
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          const SizedBox(height: 14),
          Text(
            '注入方式选择「关闭」时，本轮不会注入任何记忆。向量检索（semantic）相关能力将于后续版本提供，当前等同全量注入。',
