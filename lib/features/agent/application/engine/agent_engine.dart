...
final RegExp _kVerifyItemPattern =
    RegExp('verif|test|check|验证|测试|校验|检查|自测', caseSensitive: false);
            ...
            switch (parsePlanUpdate(call)) {
              case PlanUpdateInvalid(:final reason):
                await logControlResult(call,
                    ok: false,
                    summary: '计划参数无效',
                    detail: 'update_plan 参数无效，本次提交已忽略（已有计划保持'
                        '不变）：$reason\n请修正后全量重新提交。',
                    argSummary: '计划更新');
                budget.recordToolResult(ok: false);
