...
import 'package:aetherlink_flutter/features/agent/application/engine/agent_subagent_batch.dart';
...
class AgentEngine {
  ...
  Future<void> run(AgentTask task, AgentCancellationToken cancel) async {
    final tx = TaskTransitions(store: store, gateway: gateway, initial: task);
    final planApproval = PlanApprovalFlow(
      approval: approval,
      budget: budget,
      tx: tx,
      onNotification: onNotification,
    ...
    Future<AgentTask> save(AgentTask next) => tx.save(next);

    Future<AgentTask> transition(AgentTaskStatus status, String description) =>
    ...
    Future<bool> resolvePlanApproval(
      ...
      final restart = await planApproval.resolve(event, call, plan, cancel);
      current = tx.current;
      return restart;
    }
    ...
    try {
      ...
      // 下方直接重建审批挂起，不用模型重发一轮。
      final pendingPlanApproval = await backfillInterruptedToolCalls(
        store,
        current.id,
      ...
      while (true) {
        ...
        current = await save(
          current.copyWith(
            ...
            // 记住实测轮次：供应商不回 usage 时沿用旧值，UI 据此提示滞后。
            contextTokensRound: turn.contextTokens > 0
                ? current.rounds
                : current.contextTokensRound,
            updatedAt: DateTime.now(),
            lastEventSummary: turn.text.isNotEmpty
                ? turn.text
