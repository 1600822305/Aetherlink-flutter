...
class NetworkTtsService {
  ...
  Future<TtsSynthesisResult> _synthesizeMimo(
    ...
    final model =
        provider.model.isNotEmpty ? provider.model : 'mimo-v2.5-tts';
    final voice =
        provider.voice.isNotEmpty ? provider.voice : 'mimo_default';
    final format =
        provider.audioFormat.isNotEmpty ? provider.audioFormat : 'wav';
    ...
    if (model.contains('voicedesign') &&
      ...
      messages.add({
        'role': 'user',
        'content': provider.mimoVoiceDescription,
      });
    }
    ...
    final audioSettings = <String, dynamic>{
      'format': format,
    };
