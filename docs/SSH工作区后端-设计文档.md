...
> PTY 终端，`xterm`）、SSH-4（外部变更轮询监听）、Termux-A（一键接入·粘命令）
> 已落地并入主干；仅 Termux-B（RUN_COMMAND 全自动）作为后续。详见 §13 进度标注。
> 目标读者：Aetherlink 开发者。
...
- **外部变更**（远端他人/进程改文件）：两条增强：
  - **轮询当前打开目录（已落地，SSH-4）**：对 UI 浏览过的目录做 SFTP 重列 diff，发出 created/deleted/modified；见 §13 SSH-4。
  - 远端 `inotifywait`（若装了 inotify-tools）经 shell channel 推流 → 真正的远端文件监听（这正是 §canWatch 注释里预留的「Termux/SSH 真 inotify」）。**后续升级**。
...
- **SSH-4 外部监听** ✅（轮询版）：`RemoteSshBackend` 内置轮询器，仅跟踪 UI 经 `listDir` 浏览过的目录（`searchFiles` 走 SFTP 直读不计入），按 `_kPollInterval`(5s) 重列快照 diff，发出 created/deleted/modified（文件按 size+mtime，目录忽略自身 mtime 抖动），复用既有 `watch()` 事件总线 → 文件树/编辑器自动刷新外部改动（终端命令/协作者/其他进程）。仅在有 `watch` 监听且连接存活时轮询（不从轮询器强制重连），LRU 上限 `_kMaxWatchedDirs`(64)；diff/snapshot 抽为纯静态方法并单测。`inotifywait` 实时推流列为后续升级（检测到可用时替代轮询）。
- **Termux-A 一键接入（粘命令）** ✅：app 生成密钥 + setup 脚本；Termux 入口引导一行命令自动装 openssh/写公钥/起 sshd/保活 → 自动建 127.0.0.1:8022 密钥认证工作区（底层复用 `RemoteSshBackend`）。见 §10.5 方式 A。
  - 实现注记：① **密钥生成**：dartssh2 无 keygen API，改用 `pinenacl`（dartssh2 内部就用它做 `ssh-ed25519`，已在 lock，显式 pin `0.6.0`）生成 ed25519 对，再用纯 Dart 手工拼出 OpenSSH PEM 私钥 + `authorized_keys` 行（`domain/ssh_keygen.dart`，**dartssh2-free**、纯函数好单测；单测用 `SSHKeyPair.fromPem` 回环验证我们生成的 PEM 真能被 dartssh2 解析并签名）。这样不破坏「仅 `remote_ssh_backend.dart` 可 import dartssh2」的边界守护。② **脚本**：`domain/termux_setup.dart` 纯字符串拼装，提供两种投递（§10.5「脚本可内置/分享」）——自包含 base64 一行命令 `echo <b64> | base64 -d | bash`（完全离线，公钥内嵌脚本里）+ 「分享脚本文件」（写临时 `.sh` 经 `ShareApi` 分享）。③ **检测**：`MainActivity` 新增 `aetherlink/termux` MethodChannel 用 `PackageManager` 查 `com.termux`（manifest `<queries>` 放行 com.termux），区分 F-Droid/未知（支持）vs Play 版（已废弃→告警）；Dart 侧走 `core/platform` 抽象（`TermuxApi`/`PluginTermuxApi`/`termuxApiProvider`，ADR-0007）。④ **入口**：`open_workspace_sheet` 的 Termux `_ComingSoonTile` 占位换成真入口 → `termux_setup_sheet`（检测→展示命令/分享→「完成/测试连接」自动 probe/TOFU(localhost 自信任)/建连接+凭据+`backendType=termux` 工作区并切入）。⑤ **复用**：把连接持久化/开工作区抽成 `application/ssh_workspace_setup.dart`（`persistSshConnection`/`openAndSwitchSshWorkspace`），SSH 连接表单与 Termux 流程共用，不复制粘贴；凭据仍走已排除备份的 `workspace_ssh_credentials`（§5.2）。⑥ 用户名传 `termux`（Termux sshd 不校验用户名，按密钥认证）。⑦ 单测覆盖 keygen 回环、脚本/一行命令拼装、连接创建。
