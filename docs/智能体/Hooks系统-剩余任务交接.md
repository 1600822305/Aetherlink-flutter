...
**当前事件（17 个）**：`taskStart` / `userPromptSubmit` / `turnStart` / `preToolUse` /
`permissionRequest` / `permissionDenied` / `postToolUse` / `postToolUseFailure` /
`notification` / `fileChanged` / `turnEnd` / `stop` / `subagentStart` /
`subagentStop` / `taskEnd` / `preCompact` / `postCompact`。
...
### ⏸️ 8.（低优先，已与用户确认：暂缓，日后再做）其余协议/基础设施差距

> **2026-07-19 与用户确认**：本节全部暂缓，日后点名再做。如要从中挑，建议只做
> `updatedMCPToolOutput`（清单里唯一「价值中」的项）；其余收益小不值得。
> 下一步已转向新工程线：《上下文压缩-对比与升级计划.md》。
...
- ~~**`updatedMCPToolOutput`**（postToolUse 改写工具输出）~~：✅ **2026-07-19 已完成**——
  stdout JSON `updatedMCPToolOutput`（字段名对齐 CC）：字符串直用、其他 JSON 值序列化；
  仅 postToolUse 生效（postToolUseFailure 无此字段，同 CC）；block 裁决下丢弃；多条 hook
  取首个非空；改写事实落时间线；additionalContext / 反馈仍追加在改写后的输出之后。
...
| 2026-07-19 | ⑧ 与用户确认暂缓（日后点名再做）；现状快照事件数更新为 17；新立《上下文压缩-对比与升级计划.md》台账 | a7267bf0 |
| 2026-07-19 | ⑧ 中用户点名的 updatedMCPToolOutput 完成（postToolUse 改写工具输出，与 updatedInput 对称） | c234628a |
