...
| 2026-07-19 | 剩余任务 ⑧-updatedMCPToolOutput | ✅ | （待回填） | 输出协议新增 updatedMCPToolOutput（对标 CC，与 updatedInput 对称）：postToolUse hook 经 stdout JSON 改写回给模型的工具输出（字符串直用、其他 JSON 值序列化）；仅 postToolUse 生效（postToolUseFailure 无此字段，同 CC）；block 裁决下丢弃；多条 hook 取首个非空；改写事实落时间线；additionalContext / 反馈追加在改写后的输出之后 |
