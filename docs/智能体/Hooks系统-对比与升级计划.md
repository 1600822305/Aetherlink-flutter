...
| PostToolUse `updatedMCPToolOutput`（改写工具输出） | ✅ | ✅（2026-07-19 补齐） | 小 |
...
| 2026-07-19 | 差距补齐 ⑤ | ✅ | 0ce3b323 | asyncRewake（command 型配置字段，对标 CC）：hook 直接转后台不阻塞主链，后台跑完若阻断（退出码 2）把反馈作为排队消息注入任务，引擎在安全点消费叫醒模型（任务已结束时留待续跑进上下文）；时间线落「转后台→后台完成/后台阻断·反馈已注入」；设置页 command 型加 asyncRewake 开关 |
| 2026-07-19 | 剩余任务 ⑥ | ✅ | fb696966 | disableAllHooks 全局开关（对标 CC settings）：App 级布尔设置（agent_hooks_settings.dart，默认 false），执行层 _hooks() / runUserPromptSubmitHooks 统一短路（所有事件不执行，配置与信任状态不变，试跑不受限）；开且本有配置时落时间线提示；设置页顶部警告色总开关 |
| 2026-07-19 | 剩余任务 ⑦-1 | ✅ | 2da6aa06 | 新事件 permissionRequest / permissionDenied（对标 CC 审批门前后）：审批门 evaluate 拆 _evaluateBase + 包装层，仅基础裁决为 needsUser（即将弹审批）时跑 permissionRequestVerdict（allow → 免审直通、越 root 硬约束不可覆盖；block → forbid 强制拒绝；其余照常审批，与 preToolUse 不重复裁决）；用户拒绝审批后 fire-and-forget 跑 runPermissionDeniedHooks（拒绝原因经 tool_response 传入）；设置页 TOOL 阶段新增两事件 + 试跑支持 |
| 2026-07-19 | 剩余任务 ⑦-2 | ✅ | 7708017c | 新事件 notification（对标 CC Notification，观测型不阻断）：引擎 onNotification 回调在审批挂起（type=approval）/ ask_user 等待（type=question）时触发，task runner 接 runNotificationHooks（fire-and-forget）；matcher 匹配通知类型、pattern 忽略；消息经 stdin JSON message / notification_type 及 AETHER_MESSAGE / AETHER_NOTIFICATION_TYPE 传入；设置页 TOOL 阶段新增事件 + 试跑支持 |
| 2026-07-19 | 剩余任务 ⑦-3 | ✅ | 6f96ff12 | 新事件 preCompact / postCompact（对标 CC，观测型不阻断）：引擎 _maybeCompact 新增 onPreCompact（摘要生成前）/ onPostCompact（摘要落库后，带摘要）回调，task runner 接 runCompactionHooks（fire-and-forget）；matcher 匹配触发方式（目前仅 auto）、pattern 忽略；摘要经 stdin JSON tool_response 传入；设置页 AGENT 阶段新增两事件 |
| 2026-07-19 | 剩余任务 ⑦-4 | ✅ | 791b27eb | 新事件 fileChanged（对标 CC FileChanged，观测型不阻断，用户确认上文件监听）：复用后端 WorkspaceBackend.watch() 广播流，去抖纯逻辑在领域层 agent_file_watch.dart（按路径合并 + 静默窗口 500ms，新建后即删抵消），watcher 组装在 agent_file_watch_access.dart（订阅 + 定时冲刷，生命周期随任务 start/stop，无配置时不订阅）；matcher 匹配变更类型、pattern 匹配文件路径；路径经 file_path、变更类型经 event / AETHER_FILE_EVENT 传入；设置页 TOOL 阶段新增事件 + 试跑支持 |
| 2026-07-19 | 剩余任务 ⑧-updatedMCPToolOutput | ✅ | c234628a | 输出协议新增 updatedMCPToolOutput（对标 CC，与 updatedInput 对称）：postToolUse hook 经 stdout JSON 改写回给模型的工具输出（字符串直用、其他 JSON 值序列化）；仅 postToolUse 生效（postToolUseFailure 无此字段，同 CC）；block 裁决下丢弃；多条 hook 取首个非空；改写事实落时间线；additionalContext / 反馈追加在改写后的输出之后 |
