...
| 2026-07-19 | 剩余任务 ⑦-3 | ✅ | （本提交） | 新事件 preCompact / postCompact（对标 CC，观测型不阻断）：引擎 _maybeCompact 新增 onPreCompact（摘要生成前）/ onPostCompact（摘要落库后，带摘要）回调，task runner 接 runCompactionHooks（fire-and-forget）；matcher 匹配触发方式（目前仅 auto）、pattern 忽略；摘要经 stdin JSON tool_response 传入；设置页 AGENT 阶段新增两事件 |
