...
- 实施记录（#815）：风格库内置在 `deck_style.dart`（12 风格 + 内联自定义，新增只读工具 `pptx_styles`）；布局引擎 `deck_layout_engine.dart`（4 页型 + 7 内容布局 + 6 卡片）；图表 9 原生 OOXML + 6 形状合成 infographic；shape 新增 pie；技能升 1.5.0。**遗留**：整 deck 截图导出 PNG（视觉自检）未做，并入 M8。AI 反馈的两个高优缺陷已在 #818 补齐：image 元素支持 `src`（http(s) URL / 工作区路径，导出时自动内联，≤10MB）；新增 `pptx_edit` 以工作区 .deck.json 为源按 ops 增量编辑（set_meta/set_slide/insert_slide/remove_slide/move_slide/set_element/append_element/remove_element），写回源并可 export 覆盖重导出，技能升 1.6.0。注意：M6 的 pptx_edit 指编辑**已有 pptx 文件/模板**，deck 源级增量编辑已由 #818 覆盖。
