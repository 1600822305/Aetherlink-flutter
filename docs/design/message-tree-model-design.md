...
8. **PR-6 轻量版（分支管理列表）已落地。** 对应 cherry `TopicMessageFlow` 画布
   （`src/renderer/components/chat/messages/flow/`，基于 React Flow）的**功能等价、轻量呈现**：
   - **入口**：新增 `TopToolbarComponent.branchManagerButton`（GitBranch 图标，顶部工具栏，
     可在 DIY 布局里增删/摆放）。
   - **sheet**：`branch_manager_sheet.dart` —— 纯函数 `buildBranchTree(messages, rootId,
     activeNodeId)` 把整棵树 DFS 成缩进列表，标 `isActive/isOnActivePath/isInactiveBranch`；
     统计 `branchCount`（直译 cherry `countBranchPaths`：叶子数>1 取叶子数否则 0）+ `nodeCount`。
     节点预览用 `getMessageBlocksByIds` 一次性批量取首个 main_text。点节点 = `switchToBranch`
     （设为活动叶子，对话跳到该分支）。图例：用户/助手/当前/已禁用。
   - **与完整画布的差异（功能一致，呈现不同）**：缩进树列表 vs 自由节点图；不做平移缩放/
     连线/小地图。这些纯属视觉/操作糖，分支查看与切换能力等价。完整画布留后续（Flutter 无
     React Flow，需 `InteractiveViewer + CustomPaint` 自绘或引 `graphview`）。
   - **测试**：`buildBranchTree` 单测（线性 0 分支 / 叉路已禁用标记 + 叶子计数 / 无活动节点 /
     空话题）。
