...
**可行性更正**：MT 既提供 APK MCP，其工具 schema 就是**可发现的**——MCP `tools/list` 给出每个工具的入参 JSON Schema（字段名/类型/描述），响应字段调一次即可观测。且 MT 对外的 **MCP JSON 字段名是可读的**（混淆的只是内部 Java 类名，非对外契约）。因此「对齐 MT 字段名」既可行、也不会引入混淆命名。

**立场（推荐，默认）：以 MT 的对外字段名为准做对齐**，把它作为我们的**唯一契约**（写进 `dexcore.h` 旁）。既然要「对齐 MT / drop-in」，就直接采用 MT 的可读字段名，避免两套命名并存；我们已有的字段名若与 MT 冲突，**统一改成 MT 的**（仍是干净的单一命名，不留别名）。
- 契约铁律：**core 直接产出最终字段名 + locator，Dart 原样解析，全链路零改名兜底**（根治 manifest 空/className 空这类三层错位）。
- 「干净」与「对齐 MT」不冲突：对齐＝单一采用 MT 字段名，不是「我们的名 + MT 的名都留」。

**落地方式（拿到 MT 真实 schema）**：任选其一——
1. 用户把 MT APK MCP 的 `tools/list` 输出（各工具入参 schema）+ 每个工具一条真实响应 JSON 贴给我；
2. 或让我连到 MT 的 MCP 端点（若可达）直接 dump。
拿到后据此把 §11.14 各工具的请求/响应字段**逐字定死**，作为实现基线。下面先给一版按现有实现整理的草案，待 MT 真实 schema 到手后校正为准。
