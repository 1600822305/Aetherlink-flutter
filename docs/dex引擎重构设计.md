...
**推荐路线（未最终拍板，倾向此项）**：**先走「快车道」= 只做 §7.1 / P1**：
在现有 C++ 引擎上让会话「解析一次常驻 + 打开时建索引（含 string→class 一次）+ 搜索查索引」。
- 收益：拿到 MT 级秒搜，功能不减；
- 不做：工程拆分(P0)、契约重写(P2)、多端 C ABI/FFI、写回移植(P4) —— 这些是「干净/多端/云端可测」的收益，**按需再排**，不是性能所需。
- 顺带修：当前 P0 bug「class/method/field 搜 login 返 0」。

**第一步动作（新会话可直接开工）**：
1. 在这台开发机上把纯 C++ 引擎（`dex_parser`/`smali_disasm` 等，不含 `jni.h`）用 g++ 单独编译，
   用真实/生成的 DEX 跑通「解析一次 + 建索引 + 查索引」的原型，云端验证延迟下降与结果正确；
2. 再把常驻会话 + 索引落到 `MultiDexSessionOps` / JNI（native，需用户本地编 APK 复测）。

**未定项（需用户拍板）**：是否要「多端 + 独立引擎」。要 → 才启动 P0/P2/C ABI/FFI 大重构；
不要 → 快车道即止。用户上次倾向「先解决延迟，多端以后再说」，但未最终确认（换号继续）。
